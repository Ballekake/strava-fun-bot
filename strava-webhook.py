...
async def strava_event(request: Request):
    ...
    if payload.get('object_type') == 'activity' and payload.get('aspect_type') == 'create':
        ...
        if STRAVA_ACCESS_TOKEN:
            async with httpx.AsyncClient() as client:
                r = await client.get(f'https://www.strava.com/api/v3/activities/{activity_id}', headers={'Authorization': f'Bearer {STRAVA_ACCESS_TOKEN}'})
                if r.status_code == 200:
                    ...
                    update_resp = await client.put(f'https://www.strava.com/api/v3/activities/{activity_id}', headers={'Authorization': f'Bearer {STRAVA_ACCESS_TOKEN}'}, data=update_data)
